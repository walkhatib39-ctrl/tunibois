...
import { exportHero, exportHeroHighlights } from "@/content/pages/export";
...
export function ExportHero({ locale }: ExportHeroProps) {
  return (
    <section className="overflow-hidden bg-brand text-white">
      <Container className="grid gap-10 py-12 sm:py-14 lg:grid-cols-[0.94fr_1.06fr] lg:items-center">
        <div className="max-w-4xl">
          <h1 className="text-4xl font-semibold leading-tight sm:text-5xl">{exportHero.title[locale]}</h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-white/76">{exportHero.text[locale]}</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button href={`${route("contact", locale)}#quote`}>{exportHero.primary[locale]}</Button>
            <Button href="#process-export" variant="outline" className="border-white/35 text-white hover:bg-white/10">
              {exportHero.secondary[locale]}
        ...
        <div className="relative min-h-[320px] overflow-hidden rounded-lg border border-white/14 bg-white/8 shadow-sm sm:min-h-[430px]">
          <Image
            src={assets.woodenPallets.src}
            alt={assets.woodenPallets.alt}
            fill
            priority
          ...
          <div className="absolute inset-x-0 bottom-0 p-5">
            <p className="max-w-lg text-sm font-semibold leading-6 text-white/88">{exportHero.imageCaption[locale]}</p>
          </div>
        </div>
      </Container>
    </section>
